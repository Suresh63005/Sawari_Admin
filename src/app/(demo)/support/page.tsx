  ...
  const fetchOpenTickets = async () => {
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.response?.data?.message || error.message || 'Failed to fetch tickets',
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.response?.data?.message || error.message || 'Failed to update ticket status',
        confirmButtonColor: '#d33',
      });
    }
  };
        ...
        <div className="flex items-center space-x-4">
          ...
          {statusOptions.map((status) => (
            <Button
              ...
              className={selectedStatus === status ? 'bg-primary text-card mt-5' : 'bg-card text-primary mt-5'}
              onClick={() => {
        ...
        <DialogContent>
          ...
          <DialogFooter>
            {selectedTicket?.status === 'open' && (
              <Button onClick={() => handleUpdateStatus(selectedTicket.id, 'in_progress')}>Start</Button>
            )}
            {selectedTicket?.status === 'in_progress' && (
              <>
                <Button onClick={() => handleUpdateStatus(selectedTicket.id, 'resolved')} className="mr-2">Resolve</Button>
                <Button onClick={() => handleUpdateStatus(selectedTicket.id, 'closed')}>Close</Button>
              </>
            )}
