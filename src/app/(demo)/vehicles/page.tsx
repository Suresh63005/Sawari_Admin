...
export default function VehicleApproval() {
        ...
        try {
          ...
          if (driverId) {
            ...
            const data = (response.data.data || []).map((vehicle: any) => ({
              ...vehicle,
              car_photos: Array.isArray(vehicle.car_photos)
                ? vehicle.car_photos
                : [],
    ...
    try {
      if (action === "rc-verify") {
        ...
        await apiClient.post(
          `/v1/admin/vehicles/${vehicleId}/verify-insurance`,
          { verified_by: verifiedBy }
        );
        setVehicles(
          vehicles.map((v) =>
        ...
        await apiClient.post(
          `/v1/admin/vehicles/${vehicleId}/reject-insurance`,
        ...
        setVehicles(
          vehicles.map((v) =>
            v.id === vehicleId
              ? { ...v, is_approved: true, status: "active" }
              : v
          )
        );
        setSelectedVehicle((prev) =>
        ...
        setVehicles(
          vehicles.map((v) =>
            v.id === vehicleId
              ? { ...v, is_approved: false, status: "rejected" }
              : v
          )
        );
        setSelectedVehicle((prev) =>
  ...
  "
                                  ...
                                  <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                      ...
                                      <div className="space-y-1">
                                        <p className="text-sm">
                                          Brand: {selectedVehicle.car_brand}
                                        </p>
                                        <p className="text-sm">
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        <p className="text-sm">
                                          Verified By:{" "}
                                          {selectedVehicle.verified_by
                                            ? `${selectedVehicle.verified_by.name} (${selectedVehicle.verified_by.role})`
                                            : "N/A"}
                                        </p>
                                      </div>
                                    </div>
                          ...
                          vehicle.status !== "rejected" && (
                            <>
                              <Button
                                ...
                                onClick={() => {
                                  if (
                                    vehicle.rc_doc_status !== "verified" ||
                                    vehicle.insurance_doc_status !== "verified"
                                  ) {
                                    toast.error(
                                      "Please verify all documents before approving the vehicle",
                                      {
                                        style: {
                                          background: "card",
                                          color: "primary"
                        ...
                        <Button
                          ...
                          onClick={() => {
                            router.push(
                              `/drivers?driverId=${vehicle.driver_id}`
                            );
                          }}
        ...
        <DialogContent>
          ...
          {selectedImage && (
            <img
              ...
              className="w-[50vw] h-[80vh] object-contain rounded"
            />
          )}
        </DialogContent>
      </Dialog>
